...
import { Plus, Pencil, Trash2, Database, SendHorizonal } from "lucide-react";
...
export default function Admin() {
  ...
  const [sources, setSources] = useState({ twitter: true, telegram: true, reddit: true, news: true });
  const [weights, setWeights] = useState({ twitter: 0.6, telegram: 0.5, reddit: 0.4, news: 0.7 });
  ...
  const totalTVL = useMemo(() => protocols.reduce((a, p) => a + p.tvl, 0), [protocols]);

  function handleSaveProtocol(p: Protocol) {
    setProtocols((prev) => {
      const exists = prev.some((x) => x.id === p.id);
      const next = exists ? prev.map((x) => (x.id === p.id ? p : x)) : [{ ...p, id: cryptoId() }, ...prev];
      return next;
    });
    setOpen(false);
    setEditing(null);
          ...
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              <DialogTrigger asChild>
                <Button onClick={() => setEditing({ id: "", name: "", symbol: "", tvl: 0, collateralRatio: 150 })}>
                  <Plus /> Add Protocol
                </Button>
              </DialogTrigger>
                ...
                <div className="grid gap-4 py-2">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input id="name" value={editing?.name ?? ""} onChange={(e) => setEditing((p) => ({ ...(p as Protocol), name: e.target.value }))} />
                    </div>
                    <div>
                      <Label htmlFor="symbol">Symbol</Label>
                      <Input id="symbol" value={editing?.symbol ?? ""} onChange={(e) => setEditing((p) => ({ ...(p as Protocol), symbol: e.target.value }))} />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="tvl">TVL (USD)</Label>
                      <Input id="tvl" type="number" value={editing?.tvl ?? 0} onChange={(e) => setEditing((p) => ({ ...(p as Protocol), tvl: Number(e.target.value) }))} />
                    </div>
                    <div>
                      <Label htmlFor="cr">Collateral Ratio (%)</Label>
                      <Input id="cr" type="number" value={editing?.collateralRatio ?? 0} onChange={(e) => setEditing((p) => ({ ...(p as Protocol), collateralRatio: Number(e.target.value) }))} />
                    </div>
                  </div>
                </div>
                <DialogFooter>
              ...
              {protocols.map((p) => (
                <div key={p.id} className="rounded-xl border border-border/60 bg-card/60 p-4 shadow-sm">
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
                    <div>TVL</div><div className="text-right">${formatNum(p.tvl)}</div>
                    <div>Collateral</div><div className="text-right">{p.collateralRatio}%</div>
                  </div>
                  <div className="mt-3 flex gap-2">
                    <Button size="sm" variant="outline" className="flex-1" onClick={() => { setEditing(p); setOpen(true); }}><Pencil /> Edit</Button>
                    <Button size="sm" variant="destructive" className="flex-1" onClick={() => handleDelete(p.id)}><Trash2 /> Delete</Button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <CardContent className="space-y-5">
            {([
              ...
              <div key={key} className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label>{label}</Label>
                  <Switch checked={sources[key]} onCheckedChange={(v) => setSources((s) => ({ ...s, [key]: Boolean(v) }))} />
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-xs text-muted-foreground w-10">0</span>
                  <Slider value={[weights[key]]} onValueChange={(v) => setWeights((w) => ({ ...w, [key]: clamp01(v[0]) }))} min={0} max={1} step={0.05} />
