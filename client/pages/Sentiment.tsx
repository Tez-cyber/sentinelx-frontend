...
export default function Sentiment() {
  const [enabled, setEnabled] = useState(() => ({ twitter: true, telegram: true, reddit: true, news: true }));
  const [w, setW] = useState(() => ({ twitter: 0.6, telegram: 0.5, reddit: 0.4, news: 0.7 }));
  ...
  const score = useMemo(() => {
    ...
    const weighted = entries.reduce((acc, [k, v]) => acc + v * (w as any)[k], 0);
    const totalW = entries.reduce((acc, [k]) => acc + (w as any)[k], 0) || 1;
    return Math.round(weighted / totalW);
  }, [enabled, w]);

  const trend = useMemo(() => Array.from({ length: 30 }).map((_, i) => ({ t: `${i}`, v: Math.round(50 + 10 * Math.sin(i / 4) + (Math.random() * 6 - 3)) })), []);
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            {([
              ...
              <div key={key} className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label>{label}</Label>
                  <Switch checked={(enabled as any)[key]} onCheckedChange={(v) => setEnabled((s) => ({ ...s, [key]: Boolean(v) }))} />
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-xs text-muted-foreground w-10">0</span>
                  <Slider value={[(w as any)[key]]} onValueChange={(v) => setW((prev) => ({ ...prev, [key]: clamp01(v[0]) }))} min={0} max={1} step={0.05} />
                  <span className="text-xs text-muted-foreground w-10 text-right">1</span>
                  <span className="text-xs text-foreground w-8 text-right">{(w as any)[key].toFixed(2)}</span>
                </div>
              </div>
            ))}
        ...
        <Card className="rounded-2xl shadow-lg">
          ...
          <CardContent className="text-sm text-muted-foreground">
            Customize how SentinelX weights different sentiment sources. Your settings personalize the dashboard and alerts; they do not affect global model weights.
          </CardContent>
        </Card>
      </div>
    </Layout>
